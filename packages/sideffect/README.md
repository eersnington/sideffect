...
`ctx.env` is typed from the consumer's merged `Cloudflare.Env`, so bindings generated by `wrangler types` are available in step callbacks without local casts.
