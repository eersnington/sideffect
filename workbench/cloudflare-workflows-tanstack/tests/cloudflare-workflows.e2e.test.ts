...
test("TanStack Start dev server runs the shared Sideffect workflow matrix", async () => {
  ...
  if (!baseUrl) {
    throw new Error(
      "Vite did not expose a local dev server URL for the TanStack workflow E2E fixture.",
    );
  }

  const cases = await fetchJson<Array<E2eCase>>(`${baseUrl}/api/e2e/workflows`);
  ...
  for (const e2eCase of cases) {
    const id = `sideffect-tanstack-${e2eCase.key}-${Date.now()}`;
    await fetchJson(`${baseUrl}/api/e2e/workflows/${e2eCase.key}?id=${id}`, { method: "POST" });
...
test("TanStack Start build emits shared workflow config and Wrangler dry-run accepts it", () => {
...
async function pollWorkflowStatus(
  ...
  await vi.waitFor(
    async () => {
      latest = await fetchJson<WorkflowStatus>(`${baseUrl}/api/e2e/workflows/${key}?id=${id}`);
